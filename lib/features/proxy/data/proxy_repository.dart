...
class ProxyRepositoryImpl with ExceptionHandler, InfraLogger implements ProxyRepository {
  ...
  @override
  Stream<Either<ProxyFailure, List<ProxyGroupEntity>>> watchProxies() {
    return singbox.watchGroups().handleError((error, StackTrace? stackTrace) {
      loggy.warning("Error in watchGroups stream: $error", error, stackTrace ?? StackTrace.current);
    }).map((event) {
      if (event.isEmpty) {
        loggy.warning("Received empty groups list");
        return <ProxyGroupEntity>[];
      }
      
      loggy.info("Received ${event.length} groups from watchGroups");
  ...
  @override
  Stream<Either<ProxyFailure, List<ProxyGroupEntity>>> watchActiveProxies() {
    return singbox.watchActiveGroups().handleError((error, StackTrace? stackTrace) {
      loggy.warning("Error in watchActiveGroups stream: $error", error, stackTrace ?? StackTrace.current);
    }).map((event) {
      if (event.isEmpty) {
        loggy.warning("Received empty active groups list");
        return <ProxyGroupEntity>[];
      }
      
      loggy.info("Received ${event.length} active groups");
