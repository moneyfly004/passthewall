...
class ActiveProxyNotifier extends _$ActiveProxyNotifier with AppLogger {
  ...
  Stream<ProxyItemEntity> build() async* {
    ...
    yield* ref.watch(proxyRepositoryProvider).watchActiveProxies().handleError((error, StackTrace? stackTrace) {
      loggy.warning("Error in watchActiveProxies, but continuing", error, stackTrace ?? StackTrace.current);
    }).map((event) {
      _retryCount = 0;
      return event.getOrElse((l) {
        loggy.warning("Error getting active proxies: $l");
        return <ProxyGroupEntity>[];
      });
    }).where((groups) => groups.isNotEmpty).map((groups) {
      final firstGroup = groups.first;
      if (firstGroup.items.isEmpty) {
        loggy.warning("First group has no items");
        throw const ServiceNotRunning();
      }
      return firstGroup.items.first;
    }).handleError((error, StackTrace? stackTrace) {
      loggy.warning("Error processing active proxy, retrying...", error, stackTrace ?? StackTrace.current);
      if (_retryCount < 3) {
        _retryCount++;
        ...
        loggy.error("Max retry count reached, stopping retry");
        _retryCount = 0;
      }
    });
  }
