...
class ConnectionNotifier extends _$ConnectionNotifier with AppLogger {
  ...
  Stream<ConnectionStatus> build() async* {
    ...
    yield* _connectionRepo.watchConnectionStatus().doOnData((event) {
      ...
      loggy.info("🔄 Connection status updated: ${event.format()}");
    }).doOnError((error, stackTrace) {
      loggy.error("❌ Connection status stream error", error, stackTrace);
    });
  }
  ...
  Future<void> _connect() async {
    ...
    if (activeProfile == null) {
      loggy.info("no active profile, checking available profiles");
      try {
        final profileRepo = ref.read(profileRepositoryProvider).requireValue;
        final profilesResult = await profileRepo.watchAll().first;

        final profiles = profilesResult.getOrElse((l) => []);
        if (profiles.isNotEmpty) {
          final firstProfile = profiles.first;
          loggy.info("setting first profile as active: ${firstProfile.name}");
          await profileRepo.setAsActive(firstProfile.id).run();
          activeProfile = firstProfile;
        } else {
          loggy.warning("no profiles available to connect");
          state = AsyncError(Exception("没有可用的配置，请先添加订阅"), StackTrace.current);
          return;
        }
      } catch (e) {
        loggy.warning("failed to auto-select profile", e);
        state = AsyncError(Exception("无法自动选择配置: $e"), StackTrace.current);
        return;
      }
    }
