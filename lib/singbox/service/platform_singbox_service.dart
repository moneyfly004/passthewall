...
class PlatformSingboxService with InfraLogger implements SingboxService {
  ...
  Future<void> init() async {
    ...
    final status = statusChannel.receiveBroadcastStream().map((event) {
      loggy.debug("Status event received: $event");
      return SingboxStatus.fromEvent(event);
    });
    final alerts = alertsChannel.receiveBroadcastStream().map((event) {
      loggy.debug("Alert event received: $event");
      return SingboxStatus.fromEvent(event);
    });

    _status = ValueConnectableStream(Rx.merge([status, alerts])).autoConnect();
    await _status.first;
    loggy.debug("Initial status received: ${_status.value}");
  }
