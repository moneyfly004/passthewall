...
@riverpod
  ...
  return GoRouter(
    ...
    redirect: (context, state) {
      final introRedirect = notifier.redirect(context, state);
      if (introRedirect != null) {
        return introRedirect;
      }
      ...
      final isRegisterPage = state.uri.path == const RegisterRoute().location;
      final isForgotPasswordPage = state.uri.path == const ForgotPasswordRoute().location;
      final isIntroPage = state.uri.path == const IntroRoute().location;

      if (!isAuthenticated && !isLoginPage && !isRegisterPage && !isForgotPasswordPage && !isIntroPage) {
...
class RouterListenable extends _$RouterListenable
  ...
  Future<void> build() async {
    ...
    ref.listen(authNotifierProvider, (_, __) {
      loggy.debug("auth state changed, triggering listener");
