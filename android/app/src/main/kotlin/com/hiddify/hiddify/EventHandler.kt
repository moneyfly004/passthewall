    ...
    override fun onAttachedToEngine(flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
        ...
        statusChannel!!.setStreamHandler(object : EventChannel.StreamHandler {
            override fun onListen(arguments: Any?, events: EventChannel.EventSink?) {
                Log.d(TAG, "Flutter started listening to status channel")
                statusObserver = Observer { status ->
                    Log.d(TAG, "📡 Status changed: $status")
                    try {
                        val map = mapOf("status" to status.name)
                        events?.success(map)
                        Log.d(TAG, "✅ Status sent to Flutter: ${status.name}")
                    } catch (e: Exception) {
                        Log.e(TAG, "❌ Error sending status to Flutter: ${e.message}", e)
                    }
                }
                val currentStatus = MainActivity.instance.serviceStatus.value
                Log.d(TAG, "Current status on listen: $currentStatus")
                if (currentStatus != null) {
                    statusObserver?.onChanged(currentStatus)
                }
