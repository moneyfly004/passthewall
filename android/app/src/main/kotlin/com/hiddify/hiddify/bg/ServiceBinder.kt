...
class ServiceBinder(private val status: MutableLiveData<Status>) : IService.Stub() {
    ...
    init {
        status.observeForever {
            android.util.Log.d("ServiceBinder", "Status changed to: $it (ordinal: ${it.ordinal})")
            broadcast { callback ->
                try {
                    callback.onServiceStatusChanged(it.ordinal)
                    android.util.Log.d("ServiceBinder", "Status broadcasted to callback")
                } catch (e: Exception) {
                    android.util.Log.e("ServiceBinder", "Error broadcasting status: ${e.message}", e)
                }
            }
        }
    }
